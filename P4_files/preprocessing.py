...
# Preprocessing mail.txt for 4.5
with open("mail.txt", "r", encoding="utf-8") as f:
    mail_text = f.read()

mail_words = preprocesamiento_words(mail_text)
mail_processed = ' '.join(mail_words)
vectorizer = TfidfVectorizer(vocabulary=voca)
vectorizer.fit(email_process)  
mail_vector = vectorizer.transform([mail_processed]).toarray()  # shape: (1, 2000)

np.savez("mail.npz", data=mail_vector)